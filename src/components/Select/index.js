...
// Composant Select pour sélectionner des options
const Select = ({
  selection,
  onChange,
  name,
  ...
  const [collapsed, setCollapsed] = useState(true); // État pour gérer l'ouverture/fermeture du sélecteur

  // Fonction pour changer la valeur sélectionnée
  const changeValue = (newValue) => {
    setValue(newValue);
    setCollapsed(true); // Fermer le sélecteur après la sélection
    onChange(newValue); // Appeler la fonction de rappel avec la nouvelle valeur
  };
  ...
  return (
    <div className={`SelectContainer ${type}`} data-testid="select-testid">
      ...
      <div className="Select">
        <ul>
          {/* Affichage du titre du sélecteur */}
          <li className={collapsed ? "SelectTitle--show" : "SelectTitle--hide"}>
            {value || (!titleEmpty && "Toutes")}
          </li>
          
          {/* Liste des options lorsque le sélecteur est ouvert */}
          {!collapsed && (
            <>
              ...
              {/* Options basées sur la sélection */}
              {selection.map((s) => (
...
Select.propTypes = {
  selection: PropTypes.arrayOf(PropTypes.string).isRequired, // Tableau des options de sélection
  onChange: PropTypes.func, // Fonction appelée lors du changement de sélection
  name: PropTypes.string, // Nom du champ caché
  titleEmpty: PropTypes.bool, // Indique si l'option "Toutes" doit être affichée
  label: PropTypes.string, // Label affiché au-dessus du sélecteur
  type: PropTypes.string, // Type du sélecteur pour les styles
};

// Valeurs par défaut pour les propriétés
Select.defaultProps = {
  onChange: () => null, // Fonction vide par défaut
  titleEmpty: false, // Ne pas afficher l'option "Toutes" par défaut
  label: "", // Pas de label par défaut
  type: "normal", // Type par défaut
  name: "select", // Nom par défaut du champ caché
};
